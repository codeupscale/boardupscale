...
describe("IssuesController", () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        {
          provide: ResolveProjectPipe,
          useValue: { transform: jest.fn((v) => v) },
        },
        {
          provide: ResolveProjectBodyInterceptor,
  ...
  describe("GET /issues", () => {
    it("should return paginated issues with meta", async () => {
      ...
      issuesService.findAll.mockResolvedValue({
        items: issues,
        total: 1,
        page: 1,
        limit: 20,
    ...
    it("should calculate totalPages correctly", async () => {
      ...
      const result = await controller.findAll(TEST_IDS.ORG_ID, {
        page: 1,
        limit: 10,
      } as any);
  ...
  describe("DELETE /issues/:id", () => {
    it("should soft delete an issue", async () => {
      ...
      expect(issuesService.softDelete).toHaveBeenCalledWith(
        TEST_IDS.ISSUE_ID,
        TEST_IDS.ORG_ID,
      );
    });
  ...
  describe("POST /issues/:id/watch", () => {
    it("should toggle watch on an issue", async () => {
      ...
      const result = await controller.toggleWatch(
        TEST_IDS.ISSUE_ID,
        TEST_IDS.ORG_ID,
        user,
      );
  ...
  describe("GET /issues/:id/children", () => {
    it("should return children issues", async () => {
      const children = [
        mockIssue({ id: "child-1", parentId: TEST_IDS.ISSUE_ID }),
      ];
      issuesService.getChildren.mockResolvedValue(children);

      const result = await controller.getChildren(
        TEST_IDS.ISSUE_ID,
        TEST_IDS.ORG_ID,
      );
  ...
  describe("POST /issues/:id/work-log", () => {
    it("should create a work log", async () => {
      ...
      const result = await controller.createWorkLog(
        TEST_IDS.ISSUE_ID,
        TEST_IDS.ORG_ID,
        user,
        dto as any,
  ...
  describe("GET /issues/:id/work-logs", () => {
    it("should return work logs", async () => {
      ...
      const result = await controller.getWorkLogs(
        TEST_IDS.ISSUE_ID,
        TEST_IDS.ORG_ID,
      );
