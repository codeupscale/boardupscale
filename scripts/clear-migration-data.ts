 ...
 *
 * Usage — run from services/worker (requires its node_modules):
 *   cd services/worker && npx ts-node src/scripts/clear-migration-data.ts [email]
 *
...
async function main() {
  ...
  const { rows: userRows } = await client.query<{ id: string; organization_id: string; email: string }>(
